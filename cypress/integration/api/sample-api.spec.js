...
it('Example API Test 1-POST', () => {
    cy.requestApi(httpMethods.POST, URL.characterCreation, user).then((response) => {
...
it('Example API Test 1-PUT', () => {
    cy.visit(URL.homepage);
    cy.wait(3000);
    cy.requestApi(httpMethods.PUT, URL.getCharacter + user.id, userUpdate).then((response) => {
...
it('Example API Test 1-DELETE', () => {
    cy.visit(URL.homepage);
    cy.wait(5000);
    cy.requestApi(httpMethods.DELETE, URL.getCharacter + user.id).then((response) => {
        expect(response.body).to.equal('ok');
        expect(response.status).to.equal(200);
    }
    );
